describe('graph', function () {
  ...
  // new test
  it('should alert the user when adding an already existing node', function () {
    graph.addNode(5);
    expect(graph.addNode(5)).to.equal('This node already exists');
  });

  // new test
  it('should alert the user when adding an already existing edge', function () {
    graph.addNode(5);
    graph.addNode(1);
    graph.addEdge(1, 5);
    expect(graph.addEdge(1, 5)).to.equal('This edge already exists');
    expect(graph.addEdge(5, 1)).to.equal('This edge already exists');
  });
});
